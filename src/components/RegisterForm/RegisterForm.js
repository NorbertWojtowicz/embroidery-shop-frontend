...
const RegisterForm = () => {
    ...
    function validateUser(newUser) {
        if (newUser.email.length === 0 || newUser.password.length === 0 || newUser.username.length === 0) {
            ...
            setError("Nazwa użytkownika nie może być dłuższa niż 50 znaków"); return false;
        } else if (!newUser.password.match("^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$")) {
            ...
            setError("Hasła nie są takie same"); return false;
        ...
        <div className="wrapper fadeInDown login-form">
            ...
            <div id="formContent">
                ...
                <form id="register-form">
                    ...
                    <input type="password" id="passwordRepeat" className="fadeIn third" name="passwordRepeat" placeholder="Powtórz hasło"/>
                    <input type="submit" onClick={(e) => registerUser(e)} className="fadeIn fourth" value="Zarejestruj"/>
                </form>

                {error.length !== 0 ? <FormError message={error}/> : ""}

                {success ?
                    <div className="alert alert-success" role="alert" style={{width: "80%", margin: "0 auto 2em auto"}}>
                        Zarejestrowano pomyślnie! <a className="underlineHover" href={"/logowanie"}>Zaloguj się</a>
                    </div>
                : ""}
